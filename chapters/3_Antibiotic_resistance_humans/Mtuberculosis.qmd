...
make_dynamic_plot(plot = p, filename = "Fig3-38")
...
df |> 
  ...
  make_table_data_tab(file_name = "Fig3-38")
```
:::
...
p <- df |>
  ...
  geom_line_interactive(aes(group=antibiotic, tooltip=tooltip_l),lwd=0.5) + 
  geom_point_interactive(aes(tooltip = tooltip_p),
                         hover_nearest = TRUE) +
...
make_dynamic_plot(plot = p, filename = "Fig3-39")
```
###### Data
```{r}
df |> 
  transmute(antibiotic,
            year,
            "% resistent" = r_perc,) |> 
  make_table_data_tab(file_name = "Fig3-39")
```
:::
