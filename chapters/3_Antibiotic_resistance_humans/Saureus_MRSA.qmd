...
make_dynamic_plot(plot = p, filename = "Fig3-19")
```
###### Data
```{r}
df |> 
  ...
  make_table_data_tab(file_name = "Fig3-19")
```
:::
...
df |> 
  ...
  make_table_data_tab(file_name = "Fig3-20")
```
:::
...
df <- data$fig_data |>  
  ...
  mutate(ci = paste0(x95_percent_lag,"-",x95_percent_lag))
```
...
df |> 
  transmute(antibiotic, year, "Total number" = tot_antal, "Resistance (%)"=r_perc, "95% CI" = ci) |> 
  make_table_data_tab(file_name = "Fig3-21")
```
:::
