  ...
  out <- ggplot2::theme_bw(base_family = selected_font, base_size = text_size) %+replace%
    ggplot2::theme(
      ...
      axis.title.x = if (use_markdown("axis.title.x")) {
        ...
        ggplot2::element_text(size = text_size + 1, vjust = -1, face = "bold")
      },
      axis.text.x = if (use_markdown("axis.text.x")) {
        ggtext::element_markdown(angle = 0, hjust = 0.5, size = text_size)
      } else {
        ggplot2::element_text(angle = 0, hjust = 0.5, size = text_size)
      },
      ...
      axis.text.y = if (use_markdown("axis.text.y")) {
        ...
        ggplot2::element_text(angle = 0, size = text_size, hjust = 1, margin = margin(r = 1, unit = "mm"))
      },
      ...
      plot.caption = if (use_markdown("plot.caption")) {
        ...
        element_text(size = text_size - 2, color = "#777777", margin = margin(t = 15), hjust = 0)
      },
      ...
      legend.text = if (use_markdown("legend.text")) {
        ...
        ggplot2::element_text(size = text_size - 1)
      },
...
add_fohm_caption <- function(caption = "Source: Public Health Agency of Sweden") {
...
theme_SweSva_facet <- function(markdown = NULL) {
  ...
  theme(
    ...
    strip.text = if (use_markdown("strip.text")) {
      ...
      element_text(margin = margin(.15, .15, .15, .15), size = 12)
    },
    strip.clip = "off",
...
theme_SweSva_facet_old <- function() {
  theme(
    strip.background = element_blank(),
...
get_ci <- function(df) {
  df |>
    ...
    na.omit() |>
    mutate(
      ci_low = as.numeric(formatC(binom::binom.confint(r_antal, tot_antal, method = "wilson")$lower * 100, format = "f", digits = 1)),
